...
describe(import.meta.file, () => {
  ...
  it("should handle parameter with overlapping names", async () => {
    const expected =
      "SELECT * FROM users WHERE phone_number = '555-555-5555' AND phone_number_country = 'US'";
    const actual = Sql.to(
  ...
  it("should handle inserting JSON column correctly", async () => {
    ...
    const sqlQuery =
      "INSERT INTO users_data (user_id, data) VALUES (:user_id, :data)";
  ...
  it("should handle array column data type", async () => {
    const expected =
      "INSERT INTO users (name, emails) VALUES ('John', ARRAY['email@email.com', 'email@email.com'])";

    const actual = Sql.to(
      "INSERT INTO users (name, emails) VALUES (:name, ARRAY[:emails])",
      {
        name: "John",
